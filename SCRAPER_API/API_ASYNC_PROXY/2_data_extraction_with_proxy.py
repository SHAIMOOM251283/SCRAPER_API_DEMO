...
class AmazonScraper:
    ...
    def fetch_page(self, page_num):
        ...
        proxy_string = f"{'.'.join(flags)}:{self.api_key}@proxy-server.scraperapi.com:8001"
        ...
        try:
            response = requests.get(url, proxies=proxies, timeout=90, verify=False)
    ...
    def save_data(self):
        ...
        if format_choice in {"json", "all"}:
            ...
            print(f"✅ Saved {len(self.product_data)} products to 'computers.json'")

        if format_choice in {"csv", "all"}:
            keys = self.product_data[0].keys()
            with open("computers.csv", "w", encoding="utf-8", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=keys)
                writer.writeheader()
                writer.writerows(self.product_data)
            print(f"✅ Saved {len(self.product_data)} products to 'computers.csv'")

        if format_choice in {"excel", "all"}:
            try:
                df = pd.DataFrame(self.product_data)
                df.to_excel("computers.xlsx", index=False)
                print(f"✅ Saved {len(self.product_data)} products to 'computers.xlsx'")
            except ImportError:
