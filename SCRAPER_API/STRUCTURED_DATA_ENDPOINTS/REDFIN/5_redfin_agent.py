...
class RedfinListings:
    ...
    def extract_property_data(self, property_url):
        print(f"Extracting Agent Url from {property_url}")
        try:
            ...
            data = r.json()
            result = data.get("redfin_agent_url")
            self.property_data.append(result)
    ...
    def extract_agent_data(self, property_data):
        print(f"Extracting Agent Data from {property_data}")
        try:
            payload = { 'api_key': self.api_key, 'url': property_data }
            r = requests.get('https://api.scraperapi.com/structured/redfin/agent', params=payload)
            data = r.json()
            self.agent_data.append(data)
        except Exception as e:
            print(f"❌ Failed to extract {property_data}: {e}")

    def scrape_all_agent_data(self):
        # DM for full code
        
        filename = 'redfin_agent_data.json'
        with open(filename, 'w', encoding='utf-8') as json_file:
            json.dump(self.agent_data, json_file, ensure_ascii=False, indent=4)
            print(f"Data saved as JSON: {filename}")

    def run(self):
        self.extract_property_urls()
        self.scrape_data_from_all_property_urls()
        self.scrape_all_agent_data()
